...
export const PostsList = () => {
  ...
  let filteredResult = result.filter((el) => {
    console.log("Фильтруем элемент: ", el);
  ...
  const postlist =
    posts && filteredResult.map((el) => <PostItem post={el} key={el.elId} />);
  ...
  return (
    <>
      <div className={classes["post_list"]}>
        ...
        {searchResult || !filteredResult.length ? (
          <Alert className="alert" message={"Ничего не найдено!"} showIcon />
        ) : null}
      </div>
    </>
  );
